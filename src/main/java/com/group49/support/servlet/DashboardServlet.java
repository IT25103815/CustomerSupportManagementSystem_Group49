...
import com.group49.support.dao.*;
...
@WebServlet("/dashboard") public class DashboardServlet extends HttpServlet{
  ...
  throws ServletException,IOException{User u=WebUtil.currentUser(req);
                                      try{long unread=notifications.unreadCount(u.id());
                                          req.setAttribute("stats",tickets.stats(u,unread));
                                          req.setAttribute("recentTickets",tickets.list(u,"","").stream().limit(6).toList());
                                          req.setAttribute("unreadCount",unread);
                                          req.getRequestDispatcher("/WEB-INF/views/dashboard.jsp").forward(req,res);}
                                      catch(Exception e){throw new ServletException("Unable to load dashboard",e);
